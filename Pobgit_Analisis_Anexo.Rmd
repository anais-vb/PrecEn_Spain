...
### Pobreza Energética según quintil de renda 
...
### Pobreza energética y nivel de ingresos equivalentes 
...
### Pobreza energética en función de la situación de ocupación de la persona de referencia del hogar
...
# Convertimos las variables a character para poder manipularlas con mayor precisión posteriormente

pobgit2 <- pobgit2 %>% mutate_at(c(
                        "P13", 
                        "P5", 
                        "A1_06", 
                        "A2_06",
...
pobgit2 <- pobgit2 %>% mutate(act_p = ifelse(P5 == "Es usted la persona de referencia", P13, ifelse(
...
sum(is.na(pobgit2$act_p)) #Hay 10 NA, por lo tanto no son significativos en relaci?n al total 

pobgit2$act_p <- as.factor(pobgit2$act_p)
levels(pobgit2$act_p) # Ahora mismo hay 22 levels que debemos recodificar a ocupados, parados, jubilados, inactivos 
...
pobgit2$act_p <- fct_explicit_na(pobgit2$act_p, na_level = "Missing")


pobgit2$act_p <- fct_collapse(pobgit2$act_p,
             ocupada =c("Baja por enfermedad o maternidad/paternidad","Trabajador/a en un negocio familiar","Trabajador/a por cuenta ajena (Contrato fijo)","Trabajador/a por cuenta ajena (Contrato temporal)","Trabajador/a por cuenta ajena (Fijo)","Trabajador/a por cuenta ajena sin contrato","Trabajador/a por cuenta propia","Trabajando en un negocio familiar","Trabajando por cuenta propia"),
             parada =c("Parado/a","Parado/a, buscando primer empleo","Parado/a, que trabajó anteriormente"),
             jubilada =c("Jubilado o retirado del trabajo","Jubilado/a o retirado/a del trabajo"),
             inactiva =c("Estudiando/realizando cursos de formación","Incapacitado/a permanente","Labores del hogar","Otra situación","Realizando sin remuneración trabajos sociales, actividades benéficas","Trabajo doméstico no remunerado"),
             other_level = "Valores perdidos")

```
Una vez hemos creado una variable simplificada, creamos una tabla de frecuencia de la nueva variable para conocer mejor su distribución: 

```{r}
tabla_indicador(x = pobgit2$act_p, 
                t = "Actividad de la persona principal en la unidad familiar")
```
A continuación, pondremos en relación esta variable con los dos indicadores consensuales de pobreza energética: 

```{r}
crear_tabla2(var1 ="temp_adecuada", #variable dependiente
            var2 ="act_p", # variable independiente
            title = "Temperatura inadecuada en el hogar y actividad de la persona de referencia de la unidad familiar", # título de la tabla 
            varname = "Actividad de la persona de referencia", # Variable independiente
            numcol = 5) # Número de valores de la variable independiente 
```
```{r}
crear_tabla2(var1 ="retrasos", #variable dependiente
            var2 ="act_p", # variable independiente
            title = "Retrasos en el pago de facturas del hogar y actividad de la persona de referencia de la unidad familiar", # título de la tabla 
            varname = "Actividad de la persona de referencia", # Variable independiente
            numcol = 5) # Número de valores de la variable independiente 
```
Finalmente creamos los gráficos que representan los datos mostrados en las dos tablas de contingencia anteriores. 

```{r}
crear_grafico_temp(x = act_p, #variable independiente
                    y = temp_adecuada,  # variable dependiente 
                    title = "Temperatura adecada en el hogar", # título del gráfico
                    xtitle = "Actividad de la persona de referencia de la unidad familiar") # título axis X 
```


```{r}
crear_grafico_ret(x = act_p, #variable independiente
                    y = retrasos,  # variable dependiente 
                    title = "Retrasos en las facturas del hogar", # título del gráfico
                    xtitle = "Actividad de la persona de referencia de la unidad familiar") # título axis X 
```

### Pobreza energética y miembros de la unidad familiar en situación de desocupación 

Para crear una variable que nos indique la existencia de una persona parada/ en situación de desocupación, utilizaremos las siguientes variables de la base de datos inicial: 
* Variable `P13` que nos indica la situación laboral de la persona que contesta la encuesta 
* El conjunto de variables `AX_06` que nos indican la situación laboral de los miembros de la unidad familiar 
...
pobgit2 <- pobgit2 %>% mutate(parado = case_when(
  P13 == "Parado/a" ~ "Sí", 
  A1_06 == "Parado/a, buscando primer empleo" ~ "Sí", 
  A1_06 == "Parado/a, que trabajó anteriormente" ~ "Sí", 
  A2_06 == "Parado/a, buscando primer empleo"~ "Sí", 
  A2_06 == "Parado/a, que trabajó anteriormente" ~ "Sí",
  A3_06 == "Parado/a, buscando primer empleo"~ "Sí", 
  A3_06 == "Parado/a, que trabajó anteriormente" ~ "Sí",
  A4_06 == "Parado/a, buscando primer empleo" ~ "Sí", 
  A4_06 == "Parado/a, que trabajó anteriormente" ~ "Sí",
  A5_06 == "Parado/a, buscando primer empleo" ~ "Sí", 
  A5_06 == "Parado/a, que trabajó anteriormente" ~ "Sí",
  A6_06 == "Parado/a, buscando primer empleo" ~ "Sí", 
  A6_06 == "Parado/a, que trabajó anteriormente" ~ "Sí",
  A7_06 == "Parado/a, buscando primer empleo" ~ "Sí", 
  A7_06 == "Parado/a, que trabajó anteriormente" ~ "Sí",
  A8_06 == "Parado/a, buscando primer empleo" ~ "Sí", 
  A8_06 == "Parado/a, que trabajó anteriormente" ~ "Sí",
  A9_06 == "Parado/a, buscando primer empleo" ~ "Sí", 
  A9_06 == "Parado/a, que trabajó anteriormente" ~ "Sí",
  A10_06 == "Parado/a, buscando primer empleo" ~ "Sí", 
  A10_06 == "Parado/a, que trabajó anteriormente" ~ "Sí",
  TRUE ~ "No" 
))


pobgit2$parado <- as.factor(pobgit2$parado)

tabla_indicador(x = pobgit2$parado,
                t = "Unidades familiares con algun miembro desocupado")


```
Una vez hemos creado la variable para identificar las unidades familiares con algun miembro en situaci´n de desocupación, pondremos en relación esta variable con nuestros indicadores de referencia de pobreza energética. 

```{r}
crear_tabla2(var1 ="temp_adecuada", #variable dependiente
            var2 ="parado", # variable independiente
            title = "Temperatura inadecuada en el hogar y hogares con alguna persona parada", # título de la tabla 
            varname = "Hogares con algun miembro desocupado", # Variable independiente
            numcol = 2) # Número de valores de la variable independiente 
```
```{r}
crear_tabla2(var1 ="retrasos", #variable dependiente
            var2 ="parado", # variable independiente
            title = "Retrasos en el pago de facturas del hogar y hogares con alguna persona parada", # título de la tabla 
            varname = "Hogares con algun miembro desocupado", # Variable independiente
            numcol = 2) # Número de valores de la variable independiente 
```
Una vez creadas las tablas, construiremos los gráficos para representar las relaciones expuestas en las tablas anteriores. 

```{r}
crear_grafico_ret(x = parado, #variable independiente
                    y = retrasos,  # variable dependiente 
                    title = "Retrasos en las facturas del hogar", # título del gráfico
                    xtitle = "Hogares con algun miembro desocupado") # título axis X 
```

```{r}
crear_grafico_temp(x = parado, #variable independiente
                    y = temp_adecuada,  # variable dependiente 
                    title = "Temperatura inadecuada en el hogar", # título del gráfico
                    xtitle = "Hogares con algun miembro desocupado") # título axis X 
```

## Dimensión 3: Características del entorno urbano 

### Pobreza energética y tipo de barrio
Para analizar esta variable usaremos la variable `U1` de la encuesta que nos indica el `Tipo de barrio`. Para poder comparar estos datos con los datos generales de la encuesta EINSFOESSA, reduciremos las respuestas para adaptarlas. 

La encuesta EINSFOESSA incluye esta variable con las siguientes categorías: 
*Zona marginal
*Barrio o zona deteriorado
*Barrio o zona en buenas condiciones
*Zona de clase de media-alta

La encuesta sobre condiciones de empleo del pueblo gitano incluye la variable con las siguientes categorías: 
*Viviendas diseminadas o zona rural  
*Suburbio, zona marginal            
*Barrio obrero deteriorado          
*Barrio antiguo deteriorado         
*Barrio obrero en buenas condiciones
*Barrio antiguo en buenas condiciones
*Zona residencial de nivel medio
*Zona residencial de clase alta

Con el objetivo de simplificar las categorías existentes, se propone la siguiente distribución: 

| Variable inicial (FSG)               	| Nueva variable (FSG)                	|
|--------------------------------------	|-------------------------------------	|
| Viviendas diseminadas o zona rural   	| NSNC                                	|
| Suburbio, zona marginal              	| Zona marginal                       	|
| Barrio obrero deteriorado            	| Barrio o zona deteriorado           	|
| Barrio antiguo deteriorado           	| Barrio o zona deteriorado           	|
| Barrio obrero en buenas condiciones  	| Barrio o zona en buenas condiciones 	|
| Barrio antiguo en buenas condiciones 	| Barrio o zona en buenas condiciones 	|
| Zona residencial de nivel medio      	| Zona de clase de media-alta         	|
| Zona residencial de clase alta       	| Zona de clase de media-alta         	|


```{r}

pobgit2$tipo_barrio <- fct_collapse(pobgit2$U1, 
                                    "Marginal" = "Suburbio, zona marginal", 
                                    "Deteriorada" = c("Barrio obrero deteriorado", "Barrio antiguo deteriorado"), 
                                    "Buenas condiciones" = c("Barrio obrero en buenas condiciones", "Barrio antiguo en buenas condiciones"), 
                                    "Clase de media-alta" = c("Zona residencial de nivel medio", "Zona residencial de clase alta"), 
                                    other_level = "Otros")

tabla_indicador(x = pobgit2$tipo_barrio, 
                t = "Tipo de barrio")

```
Una vez disponemos de una variable comparable para determinar el tipo de barrio, pondremos en relación esta variable con los dos indicadores disponibles de pobreza energética para esta encuesta. 

```{r}
crear_tabla2(var1 ="temp_adecuada", #variable dependiente
            var2 ="tipo_barrio", # variable independiente
            title = "Temperatura adecuada en el hogar según tipo de barrio", # título de la tabla 
            varname = "Tipo de barrio", # Variable independiente
            numcol = 5) # Número de valores de la variable independiente 
```
```{r}
crear_tabla2(var1 ="retrasos", #variable dependiente
            var2 ="tipo_barrio", # variable independiente
            title = "Retrasos en el pago de gastos del hogar según tipo de barrio", # título de la tabla 
            varname = "Tipo de barrio", # Variable independiente
            numcol = 5) # Número de valores de la variable independiente 
```

```{r}
crear_grafico_ret(x = tipo_barrio, #variable independiente
                    y = retrasos,  # variable dependiente 
                    title = "Retrasos en las facturas del hogar", # título del gráfico
                    xtitle = "Tipo de barrio") # título axis X 
```
```{r}
crear_grafico_temp(x = tipo_barrio, #variable independiente
                    y = temp_adecuada,  # variable dependiente 
                    title = "Temperatura adecuada en el hogar", # título del gráfico
                    xtitle = "Tipo de barrio") # título axis X 
```

### Pobreza energética y tamaño del municipio 

```{r}
summary(pobgit2$TAMANO)

# En primer lugar, modificamos el nombre de la variable para su clarificación y simplificamos las etiquetas de las categorías 

pobgit2 <- rename (pobgit2, tamano_municipio = TAMANO)

pobgit2$tamano_municipio <- fct_recode(pobgit2$tamano_municipio,  
                    "<20.000h" = "Menos de 20.000 habitantes", 
                    "20.000h-50.000h" = "Entre 20.000 y 50.000 habitantes", 
                    "50.000h - 100.000h" = "Entre 50.000 y 100.000 habitantes", 
                    ">100.000h" = "100.000 habitantes o más"
                    )

tabla_indicador(x = pobgit2$tamano_municipio, 
                t = "Tamaño del municipio")

```
A partir de la definición de las categorías, analizamos los indicadores en función del tamaño del municipio: 

```{r}
crear_tabla2(var1 ="temp_adecuada", #variable dependiente
            var2 ="tamano_municipio", # variable independiente
            title = "Temperatura adecuada en el hogar según el tamaño del municipio", # título de la tabla 
            varname = "Tamaño del municipio", # Variable independiente
            numcol = 4) # Número de valores de la variable independiente 
```
```{r}
crear_tabla2(var1 ="retrasos", #variable dependiente
            var2 ="tamano_municipio", # variable independiente
            title = "Retrasos en el pago de gastos del hogar según el tamaño del municipio", # título de la tabla 
            varname = "Tamaño del municipio", # Variable independiente
            numcol = 4) # Número de valores de la variable independiente 
```
```{r}
crear_grafico_temp(x = tamano_municipio, #variable independiente
                    y = temp_adecuada,  # variable dependiente 
                    title = "Temperatura adecuada en el hogar", # título del gráfico
                    xtitle = "Tamaño del municipio") # título axis X 
```

```{r}
crear_grafico_ret (x = tamano_municipio, #variable independiente
                    y = retrasos,  # variable dependiente 
                    title = "Retrasos en el pago de gastos del hogar", # título del gráfico
                    xtitle = "Tamaño del municipio") # título axis X 
```

## Dimensión 4: Impactos sobre la salud, relaciones sociales y calidad de vida

### Discriminación y pobreza energética 

```{r}
# Eliminamos el factor NS/NC para convertirlo en valores perdidos
levels(pobgit2$P43)[levels(pobgit2$P43)=='NS/NC'] <- NA

# Exploramos la variable sobre Sentimiento de discriminación 
tabla_indicador(x = pobgit2$P43, 
                t = "Sentimiento de discriminación")
```
Cruzamos la variable con los indicadores de pobreza energética
```{r}
crear_tabla2(var1 ="retrasos", #variable dependiente
            var2 ="P43", # variable independiente
            title = "Retrasos en el pago de gastos del hogar y sentimiento de discriminación", # título de la tabla 
            varname = "Sentimiento de discriminación", # Variable independiente
            numcol = 2) # Número de valores de la variable independiente 
```
```{r}
crear_tabla2(var1 ="temp_adecuada", #variable dependiente
            var2 ="P43", # variable independiente
            title = "Temperatura adecuada en el hogar y sentimiento de discriminación", # título de la tabla 
            varname = "Sentimiento de discriminación", # Variable independiente
            numcol = 2) # Número de valores de la variable independiente 
```
```{r}
crear_grafico_temp(x = P43, #variable independiente
                    y = temp_adecuada,  # variable dependiente 
                    title = "Temperatura adecuada en el hogar", # título del gráfico
                    xtitle = "Sentimiento de discriminación") # título axis X 
```

```{r}
crear_grafico_ret (x = P43, #variable independiente
                    y = retrasos,  # variable dependiente 
                    title = "Retrasos en el pago de gastos del hogar", # título del gráfico
                    xtitle = "Sentimiento de discriminación") # título axis X 
```
...
Con el objetivo de analizar el impacto de la pobreza energética sobre aspectos vinculados a la salud, como es la alimentación, utilizaremos la variable `P51_2` incluída en la encuesta, que recoje las unidades familiares que no pueden acceder a una comida de carne, pollo o pescado al menos cada dos días. 

Cruzamos la variable con los indicadores de pobreza energética: 

```{r}
crear_tabla2(var1 ="retrasos", #variable dependiente
            var2 ="P51_2", # variable independiente
            title = "Retrasos en el pago de gastos del hogar y afectación en la alimentación", # título de la tabla 
            varname = "Afectación en la alimentación", # Variable independiente
            numcol = 2) # Número de valores de la variable independiente 
```
...
crear_tabla2(var1 ="temp_adecuada", #variable dependiente
            var2 ="P51_2", # variable independiente
            title = "Temperatura adecuada en el hogar y afectación en la alimentación", # título de la tabla 
            varname = "Afectación en la alimentación", # Variable independiente
            numcol = 2) # Número de valores de la variable independiente 
```

```{r}
crear_grafico_temp(x = P51_2, #variable independiente
                    y = temp_adecuada,  # variable dependiente 
                    title = "Temperatura adecuada en el hogar", # título del gráfico
                    xtitle = "Afectación en la alimentación") # título axis X 
```

```{r}
crear_grafico_ret (x = P51_2, #variable independiente
                    y = retrasos,  # variable dependiente 
                    title = "Retrasos en el pago de gastos del hogar", # título del gráfico
                    xtitle = "Afectación en la alimentación") # título axis X 
```

### Dispositivos electrónicos en el hogar

Por último, veremos como las unidades familiares en situación de pobreza energética responden ante las preguntas de la encuesta relativas al uso de dispositivos electrónicos, y como este hecho, como veremos, puede ser también un eje de desigualdad en la falta de acceso a determinados servicios energéticos y de telecomunicaciones. 

Para ello, crearemos una variable sobre el acceso a dispoisitovs energéticos que incluya tres de los principales electrodomésticos y/o dispositivos electrónicos en el hogar: la lavadora (`P52_5`), la televisión (`P52_4`), el ordenador personal (`P52_6`) y el teléfono móvil (`P52_3`). 

```{r}

summary(pobgit2$P52_3)


pobgit2 <- pobgit2 %>% mutate( dispositivos = case_when(
  P52_3 == "No" ~ "No", 
  P52_4 == "No" ~ "No",
  P52_5 == "No" ~ "No",
  P52_6 == "No" ~ "No",
...
pobgit2$dispositivos <- as.factor(pobgit2$dispositivos)

summary(pobgit2$dispositivos)
...
```{r}
crear_tabla2(var1 ="retrasos", #variable dependiente
            var2 ="dispositivos", # variable independiente
            title = "Retrasos en el pago de gastos del hogar y acceso a dispositivos electrónicos", # título de la tabla 
            varname = "Acceso a dispositivos electrónicos y/o electrodomésticos básicos", # Variable independiente
            numcol = 2) # Número de valores de la variable independiente 
```
...
crear_tabla2(var1 ="temp_adecuada", #variable dependiente
            var2 ="dispositivos", # variable independiente
            title = "Temperatura adecuada en el hogar y acceso a dispositivos electrónicos ", # título de la tabla 
            varname = "Acceso a dispositivos electrónicos y/o electrodomésticos básicos", # Variable independiente
            numcol = 2) # Número de valores de la variable independiente 
```

```{r}
crear_grafico_temp(x = dispositivos, #variable independiente
                    y = temp_adecuada,  # variable dependiente 
                    title = "Temperatura adecuada en el hogar", # título del gráfico
                    xtitle = "Acceso a dispositivos electrónicos y/o electrodomésticos básicos") # título axis X 
```

```{r}
crear_grafico_ret (x = dispositivos, #variable independiente
                    y = retrasos,  # variable dependiente 
                    title = "Retrasos en el pago de gastos del hogar", # título del gráfico
                    xtitle = "Acceso a dispositivos electrónicos y/o electrodomésticos básicos") # título axis X 
```
